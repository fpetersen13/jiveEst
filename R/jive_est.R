...
#' The type of estimator (i.e. UJIVE1 vs. UJIVE2) and standard errors
...
#' @param type: UJIVE1 or UJIVE2 for the type of estimator that should be used,
...
#' @export
jive_est <- function(y,X,Z,type=UJIVE1, vcov = HCE){
  n  <- length(y);
  k  <- dim(X)[2];
  ret <- list();
  if(type==UJIVE1){
    jive <- UJIVE1(y,X,Z,vcov)
    ret$coe <- jive$coe
    ret$var <- jive$var
...
UJIVE1 <- function(y,X,Z,vcov){
  ...
  b_jive1 <- solve(t(X.hat)%*%X)%*%t(X.hat)%*%y
  ret$coe <- b_jive1
  ...
  if(vcov==HC0){
    sigma.hat2 <- sum((y - X%*%b_jive1)^2)/n
